export class omk {
  constructor(params) {
    ...
    this.vocabs = params.vocabs
      ? params.vocabs
      : // IMPORTANT : les vocabs à utiliser pour ensuite recup les props
        ["dcterms", "foaf", "skos", "jdc", "mov", "mov2"];
    this.user = false;
    this.props = [];
    this.class = [];
    this.rts;
    ...
    this.getPropTerm = function (id) {
      // trouver le term associe à l'id de la propriete
      return me.props.filter((prp) => prp["o:id"] == id)[0]["o:term"];
    };

    //github.com/samszo/THYP_23-24/blob/844c88fe371c7712f05c5b43884bea0aff900836/projets.html#L126
    this.putAleaData = function (d) {
      let aleaData = {};
      d["o:resource_template_property"].forEach((p) => {
        let prop = this.getPropTerm(p["o:property"]["o:id"]);
        aleaData[prop] = "blabla";
      });
      this.createRessource(aleaData);
    };

    this.createItem = function (data, cb = false) {
      let url =
        me.api + "items?key_identity=" + me.ident + "&key_credential=" + me.key;
